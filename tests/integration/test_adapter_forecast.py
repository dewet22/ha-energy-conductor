...
2. _slots_from_solcast now filters to tomorrow's local date only, so configuring
   the "forecast today" sensor or an aggregate sensor degrades cleanly to
   fallback rather than silently misreading the wrong day's slots.
3. daily_total_sensor stores its value as fallback_kwh rather than a synthetic
   slot stamped `now` — a daily TOTAL (already generated by evening) previously
   misrepresented itself as generation still to come, corrupting anything
   derived from the forecast timeline (e.g. the SoC projection).
"""
...
async def test_solcast_tomorrow_filter(hass, mock_config_entry, now_utc):
    ...
    back to seasonal/stats rather than silently reading the wrong day's slots.
    """
