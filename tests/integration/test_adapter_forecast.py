...
async def test_daily_sensor_uses_fallback_kwh_not_synthetic_slot(hass, mock_config_entry, now_utc):
    ...
    The old synthetic slot had start=now, which misrepresented a daily TOTAL (already
    generated by evening) as generation still to come. Stored as fallback_kwh it feeds
    total_kwh_forecast without inventing a slot on the timeline.
    """
